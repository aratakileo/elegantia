...
abstract class FabricBasedGenTask extends GenTask {
    ...
    private void generateManifest() {
        ...
        if (entriesData.entries.common != null)
            sourceData.entrypoints.put("main", ["${entriesData.platform_package}.fabric.${COMMON_ENTRY_CLASS}"])

        if (entriesData.entries.client != null)
            sourceData.entrypoints.put("client", ["${entriesData.platform_package}.fabric.${CLIENT_ENTRY_CLASS}"])
    ...
    private void generateEntryJava(@NotNull Map entriesData, @NotNull String entryKey) {
        ...
        final var isCommon = entryKey == "common"
        final var fileName = isCommon ? COMMON_ENTRY_FILE_NAME : CLIENT_ENTRY_FILE_NAME
