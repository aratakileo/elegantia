...
public class NativeGifImage implements AutoCloseable {
    ...
    public NativeGifImage(@NotNull Size2iInterface size, int frames, int[] pixels, int[] delays) {
    ...
    public int processFrames(@NotNull FrameProcessor frameProcessor) {
        ...
        for (var frame = 0; frame < getFrames(); frame++) {
            final var frameImage = new NativeImage(NativeImage.Format.RGBA, width, height, false);

            for (var x = 0; x < width; x++)
                for (var y = 0; y < height; y++)
                    frameImage.setPixelRGBA(x, y, pixels[(frame * height + y) * width + x]);

            frameProcessor.process(frame, totalDelay, new Frame(frameImage, delays[frame]));

            totalDelay += delays[frame];
        }
